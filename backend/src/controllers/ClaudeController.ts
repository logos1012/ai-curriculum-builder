...
import { claudeService, CurriculumContext } from '../lib/claude';
...
export class ClaudeController {
  ...
  async chat(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const response = await claudeService.generateResponse(message, curriculumContext);

      // 응답에서 제안 질문들을 추출 (선택적)
      const suggestions = await this.extractSuggestions(response, curriculumContext);

      logger.info(`Claude chat response generated for user ${userId}`);
      ...
      logger.error('ClaudeController.chat error:', error);
  ...
  async streamChat(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      try {
        // Claude API 스트리밍 호출
        for await (const chunk of claudeService.generateStreamResponse(message, curriculumContext)) {
          fullResponse += chunk;
        ...
        logger.info(`Claude streaming response completed for user ${userId}`);
      } catch (streamError) {
        logger.error('Claude streaming error:', streamError);
        res.write(`data: ${JSON.stringify({ 
          type: 'error', 
          message: 'AI 응답 생성 중 오류가 발생했습니다' 
        })}\n\n`);
      ...
      logger.error('ClaudeController.streamChat error:', error);
  ...
  async enhanceContent(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const enhancedContent = await claudeService.enhanceContent(content, curriculumContext);

      // 개선 사항 요약 생성
      const improvements = this.analyzeImprovements(content, enhancedContent);

      logger.info(`Content enhancement completed for user ${userId}`);
      ...
      logger.error('ClaudeController.enhanceContent error:', error);
  ...
  async generateQuestions(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const questions = await claudeService.generateClarifyingQuestions(curriculumContext);

      // 질문을 카테고리별로 분류
      const categorizedQuestions = this.categorizeQuestions(questions);

      logger.info(`Clarifying questions generated for user ${userId}`);
      ...
      logger.error('ClaudeController.generateQuestions error:', error);
  ...
  private async extractSuggestions(response: string, context: CurriculumContext): Promise<string[]> {
    try {
  ...
  private categorizeQuestions(questions: string[]): { name: string; questions: string[] }[] {
    const categories = [
    ...
    const categorized = categories.map(category => ({
      ...
      questions: questions.filter(question =>
        category.keywords.some(keyword => question.includes(keyword))
      ),
    }));
