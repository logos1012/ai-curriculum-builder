...
export class ClaudeService {
  // 시스템 프롬프트 생성
  private buildSystemPrompt(context: CurriculumContext): string {
...
응답은 마크다운 형식으로 구조화하여 제공해주세요.`
  ...
  // 일반 채팅 응답 생성
  async generateResponse(message: string, context: CurriculumContext): Promise<string> {
    try {
  ...
  // 스트리밍 응답 생성
  async *generateStreamResponse(message: string, context: CurriculumContext): AsyncGenerator<string, void, unknown> {
    try {
      const systemPrompt = this.buildSystemPrompt(context)
...
개선된 버전을 마크다운 형식으로 제공해주세요.`
  ...
  // 구체화 질문 생성
  async generateClarifyingQuestions(context: CurriculumContext): Promise<string[]> {
    try {
...
각 질문은 한 줄로 작성하고, 번호를 매겨주세요.`
      ...
      logger.info(`Generated ${questions.length} clarifying questions`)
      return questions
    } catch (error) {
      logger.error('Clarifying questions generation error:', error)
      throw error
    }
  }
}
